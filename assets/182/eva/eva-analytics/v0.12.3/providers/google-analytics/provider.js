...
define(['eva-analytics/entities/interaction-data', 'eva-analytics/entities/custom-variable'], function (InteractionData, CustomVariable) {
    ...
    GoogleAnalyticsProvider.prototype.setCustomVariable = function (customVariable) {
        ...
        _gaq.push(['whichGAP._setCustomVar', sanitisedSlot, customVariable.name, customVariable.value, sanitisedScope]);
        // debug info
        if (isDev) {
    ...
    GoogleAnalyticsProvider.prototype.trackEvent = function(interactionData) {
        ...
        var sanitisedValue = isNaN(parsedValue) ? 0 : parsedValue;
        // tell google about it
        _gaq.push(['whichGAP._trackEvent', interactionData.category, interactionData.action, interactionData.label || '', sanitisedValue, interactionData.nonInteraction]);
        // debug info
        if (isDev) {
            console.log('Track event: ' + interactionData.category + ' | ' + interactionData.action + ' | ' + interactionData.label + ' | ' + sanitisedValue + ' | ' + interactionData.nonInteraction);
        }
    }
    ...
    GoogleAnalyticsProvider.prototype.trackPageView = function (url) {
        ...
        if (typeof url === 'string' && url.length) {
            _gaq.push([trackerObjectMethodName, url]);
            if (isDev) {
                console.log('Tracking virtual page view. Url: ' + url);
            }
        } else {
            _gaq.push([trackerObjectMethodName]);
            if (isDev) {
