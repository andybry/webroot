define(['jquery', 'eva-analytics/tracking/tracking-base', 'eva-analytics/entities/interaction-data', 'eva-utils/js/shims/object'], function ($, TrackingBase, InteractionData) {
    ...
    var eventName = 'click.clickouttracking',
        linkTypeRr = new RegExp('linkType-([a-zA-Z]+)'),
        linkVRr = new RegExp('linkValue-([\\d+\\.]+)'),
        ...
        targetElementsSelector = 'a.is-trackable, a[href^="http"]';
    ...
    ClickOutTracking.prototype.setupTracking = function() {
        ...
        $('body').on(eventName, targetElementsSelector, $.proxy(function(e) {
            ...
            if (!$this.hasClass('not-trackable') && !new RegExp(this.win.document.domain, 'i').test(href) && !/adtech/i.test(href)) {
                ...
                if (dataTrackerValues !== undefined) {
                    if (linkTypeRr.test(dataTrackerValues)) {
                        linkType = dataTrackerValues.match(linkTypeRr)[1];
                    }
                    if (linkVRr.test(dataTrackerValues)) {
                        linkValue = parseInt(dataTrackerValues.match(linkVRr)[1], 10);
                    }
                }
                ...
                this.followAnchor($this, this.win, e);
    ...
    ClickOutTracking.prototype.stopTracking = function () {
